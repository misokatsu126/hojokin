...
このツールは、補助金名から探すのではなく、**これから使うお金＝支出案件**から、補助金を使える可能性・発注前の注意・今日やる申請準備・まず確認すべき定番制度・AI相談プロンプト・必要書類／証憑・期限・公式確認ログまでを整理する**補助金申請アシストツール**です。

初心者は「かんたん表示」で迷わず（発注していいか／今日やること／まず見る制度）、慣れた人は「実務者表示」で証憑・期限・公式確認・AI回答のタスク化まで扱えます。

- **サイト側で高額なAI APIを大量に使いません。** 案件情報を整理して、ユーザー自身の ChatGPT・Claude・Gemini に貼れる**高品質な相談プロンプト**を生成します。
- **Next.js 14（App Router） / TypeScript / Tailwind CSS / Supabase**
- Supabase / OpenAI が未設定でも、localStorage とルールベースで主要機能が動作します。

## 主要画面

- **ホーム**：今の結論（発注していいか・今日やること）、今日やる申請準備、支出登録CTA、AI相談の軽い紹介。
- **支出案件作成（/projects/new）**：補助金名を知らなくてもOK。支出テーマ（カテゴリ別）または業種から選ぶ／自由入力。30秒かんたん作成も可能。
- **案件詳細（/projects/[id]）**：発注判断 → 今日やる申請準備 → 足りない情報 → まず確認すべき定番制度 → 自分のAIに相談する → 相談用/見積依頼メモ → 概算 → 申請準備の進捗 → いまどの段階 → 進め方・相談先 →（実務者表示）必要書類・証憑／期限・スケジュール／公式確認ログ → 検索候補。
- **自分のAIに相談する**：補助金チェック／公式要領読み取り／見積書チェック／申請書メモ／相談文／支払い前／実績報告 のプロンプトを生成。AI回答の貼り戻し・タスク化（公式確認前の参考情報）。
- **必要書類・証憑ボックス**：申請前／発注・契約時／納品・支払い時／実績報告時＋支出テーマ別の重要証憑を管理。
- **期限・スケジュール**：公募締切・事前相談・様式発行依頼・GビズID・実績報告など。公募締切から目安日を自動提案。
- **公式確認ログ**：誰に・いつ・何を確認したかを記録（AI回答に頼り切らない）。
- **自動探索／管理者画面（/discovery）**：Jグランツ・J-Net21・ミラサポplus 等を定期収集し、案件と自動照合（運用者向け）。

> ※ 「使える」「採択される」と断定しません。最終判断は公式要領・窓口・専門家への確認が前提です。
